...
class TopCommodityCategories extends React.Component {
	...
	componentDidMount() {
		...
		axios
			.get(`${process.env.REACT_APP_BACKEND_URL}/exchange-rate`)
			.then(res => {
	...
	setPercentages = () => {
		...
		let UGX_KES_Percentage = Math.round(
			(this.state.UGX_KES_Count / totalCount) * 100
		);
		...
		let KES_USD_Percentage = Math.round(
			(this.state.KES_USD_Count / totalCount) * 100
		);
			...
			<div className="Chart">
				...
				<ResponsiveBar
					...
					axisLeft={{
						...
						tickPadding: 5,
						tickRotation: 0,
						legend: 'Percentage of Direction',
						legendPosition: 'middle',
						legendOffset: -70
					}}
					labelSkipWidth={0}
