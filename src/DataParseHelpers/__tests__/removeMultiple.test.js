...
describe("removeMultiples()", () => {
  test("Does it return array without duplicates?", () => {
    const response = removeMultiple(messedUpData);

    expect(response.dataStructure).toBeArray();
    expect(response.dataStructure)
      .not()
      .toContain(/Maize,Maize,Maize/gi);
  });
});
